...
$actuacion = \App\Models\Actuacion::join('listas', 'actuacions.id', '=', 'listas.actuacions_id')
    ...
    ->where('listas_user.user_id', Auth::id())
                    ...
                    <svg width="32px" height="32px" viewBox="0 0 24 24" version="1.1"
                        ...
                        <g id="SVGRepo_iconCarrier">
                            ...
                            <g id="🔍-System-Icons" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                <g id="ic_fluent_arrow_forward_24_regular" fill="#b06d36" fill-rule="nonzero">
                                    <path
        ...
        <div class="text-left bg-yellow-100 p-3 rounded-lg">
            ...
            @if ($actuacionesBanda->count() > 0)
                @foreach ($actuacionesBanda as $actuacion)
                    ...
                    @endphp
                    <a href="{{ route('listas.actuacion', ['actuacion_id' => $actuacion->id]) }}">
                    <p class="text-m font-semibold text-gray-800 mt-2">{{ $fecha ? Carbon::parse($fecha)->format('d/m/Y') : 'Sin fecha' }} -
                        {{ $actuacion->descripcion }} - {{ $actuacion->contrato->poblacion }} ➡</p>
                    </a>
                @endforeach
            @else
