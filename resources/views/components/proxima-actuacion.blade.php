@php
    ...
    $actuacion = \App\Models\Actuacion::join('listas', 'actuacions.id', '=', 'listas.actuacions_id')
        ...
        ->where('listas_user.user_id', Auth::user()->id)        
...
<div class="w-full max-w-md mx-auto bg-white shadow-lg rounded-lg p-4">
    <div class="grid grid-cols-2 gap-4">
        ...
        <div class="text-left bg-blue-100 p-3 rounded-lg">
            ...
            @if ($actuacion && $actuacion->disp) 
                <p class="text-sm text-gray-600 mt-2">
                    📅 {{ Carbon::parse($actuacion->fechaActuacion)->format('d/m/Y') }}
                </p>
            ...
            @if ($actuacion && !$actuacion->disp)
                ...
                <a href="{{ route('listas.actuacion', ['actuacion_id' => $actuacion->id]) }}">
                    <p class="text-l font-semibold text-gray-800 mt-2 flex items-center space-x-2 truncate">
                        ...
                        <span class="truncate">{{ $actuacion->descripcion }}</span>
                    </p> 
                </a>
            @else
                <p class="text-sm text-gray-600 mt-2">📅 {{ __('Nada a la vista') }}</p>
