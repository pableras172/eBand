@php
    ...
    $actuacion = Auth::user()->listas()
    ->wherePivot('disponible', 1)
    ->with('actuacion')
    ->get()
    ->first()?->actuacion;

    $actuacionesBanda = App\Models\Actuacion::whereBetween('fechaActuacion', [now(), now()->addDays(7)])->with('tipo')->with('contrato')->get();
@endphp
    ...
    <div class="grid grid-cols-2 gap-4">
        ...
        <div class="text-left bg-blue-100 p-3 rounded-lg">
            ...
            <p class="text-l font-semibold text-gray-800 mt-2">
                🎶 {{ $actuacion->descripcion ?? 'Sin próximas actuaciones' }}
            </p>
    ...
    <div class="grid grid-cols-2 gap-4 mt-2">
        ...
        <div class="text-left bg-yellow-100 p-3 rounded-lg">
            ...
            @if($actuacionesBanda->count() > 0)
                @foreach($actuacionesBanda as $actuacion)
                    <p class="text-lg font-semibold text-gray-800 mt-2">📍 {{ $actuacion->contrato->poblacion }}</p>
                @endforeach
            @else
                <p class="text-lg font-semibold text-gray-800 mt-2">{{__('No hay actuaciones')}}</p>
            @endif
        
            <div class="flex justify-end">
                <svg width="32px" height="32px" viewBox="0 0 24 24" version="1.1" xmlns="http://www.w3.org/2000/svg" fill="#b06d36">
                ...
                {{-- Columna derecha: Total de actuaciones del mes --}}
                <div class="text-right bg-green-100 p-3 rounded-lg">
                    <h2 class="text-sm text-gray-600">{{ __('Esta semana') }}</h2>
                    
                    <p class="text-l font-semibold text-gray-800">
                        🎭 {{ $actuacionesBanda->count() > 0 ? "Hay {$actuacionesBanda->count()} actuación(es) esta semana." : __('No hay actuaciones') }}
                    </p> 
