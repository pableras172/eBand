<x-app-layout>
    ...
    <div class="container mx-auto py-10 px-4 sm:px-8 lg:px-32">
        ...
        <form 
            @if(isset($actuacion))
                action="{{ route('actuacion.update', $actuacion->id) }}"
                method="POST"
            @else
        ...
        >
            ...
            <div class="mb-4">
                ...
                <textarea id="descripcion" name="descripcion" rows="3" required
                    class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    @if(isset($actuacion)){{ $actuacion->descripcion }}@endif
                </textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="tipoactuacions_id" name="tipoactuacions_id" required
                    ...
                    @foreach ($tipoActuacion as $tipo)
                        <option value="{{ $tipo->id }}" @if(isset($actuacion) && $actuacion->tipoactuacions_id == $tipo->id) selected @endif>{{ $tipo->nombre }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="mb-4">
                    ...
                    <input type="number" id="coches" name="coches" value="{{ isset($actuacion) ? $actuacion->coches : '' }}"
                        class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                </div>
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <input type="number" id="musicos" name="musicos" value="{{ isset($actuacion) ? $actuacion->musicos : '' }}"
                        class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                </div>
                <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="checkbox" id="pagado" name="pagado"
                    class="mt-1 focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded" {{ isset($actuacion) && $actuacion->pagado ? 'checked' : '' }}>
            </div>            
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                    <div class="max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden md:max-w-2xl">
                        ...
                        <div class="flex justify-center">
                            <a href="{{ route('actuacion.edit', $actuacion->id) }}" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest bg-gray-800 hover:bg-gray-900 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring focus:ring-gray-300 disabled:opacity-25 transition mb-2 mr-2">
                                Editar
                            </a>
