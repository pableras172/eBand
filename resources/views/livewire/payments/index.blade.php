<div class="max-w-7xl mx-auto py-2 sm:px-4 lg:px-4 space-y-2">
    ...
    <x-ui.container.table>
        <x-ui.table>
            <x-slot name="head">                
                <x-ui.table.header for-crud wire:click="sortBy('users_id')"
                    >{{ __('Nom') }}</x-ui.table.header>
            ...
            <x-slot name="body">
                ...
                <x-ui.table.row wire:loading.class.delay="opacity-75" class="{{ $payment->confirmadausuaroi ? 'bg-green-100' : 'bg-red-100' }}">
                    <x-ui.table.column for-crud
                        >{{ $payment->user->name }}</x-ui.table.column
                    >
                    <x-ui.table.column for-crud
